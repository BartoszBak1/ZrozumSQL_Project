...
INSERT INTO expense_tracker.monthly_budget_planned(year_month, budget_planned, left_budget) VALUES ('12-2020', 1000, 1000);
	...
	AS $$
	 BEGIN 
		IF (TG_OP = 'DELETE') THEN 
		  ...
		  WHERE expense_tracker.monthly_budget_planned.year_month = (EXTRACT ( MONTH FROM OLD.transaction_date) ||'-'||  EXTRACT (YEAR FROM OLD.transaction_date));
		...
		ELSIF (TG_OP = 'INSERT') THEN 
		  ...
		  WHERE expense_tracker.monthly_budget_planned.year_month = (EXTRACT ( MONTH FROM NEW.transaction_date) ||'-'||  EXTRACT (YEAR FROM NEW.transaction_date));
...
UPDATE expense_tracker.transactions SET transaction_value = -200
